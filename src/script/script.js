const Offcanvas = function () {
  ...
  function offcanvasGetAttr(selector) {
    if (selector.offcanvasContainer) {
      const offcanvasClose =
        selector.offcanvasContainer.querySelector(".offcanvas-close");
      const offcanvasWrapper =
        selector.offcanvasContainer.querySelector(".offcanvas-wrapper");
      let dataPostion =
        selector.offcanvasContainer.getAttribute("data-position");
      let dataSize = selector.offcanvasContainer.getAttribute("data-size");
      let dataTransition =
        selector.offcanvasContainer.getAttribute("data-transition");
  ...
  function offcanvasEventFunc(selector, options) {
    selector.offcanvasButton.addEventListener("click", () => {
      selector.offcanvasContainer.classList.add(
        "show",
        options.dataPostion,
        options.dataSize
      );
  ...
  function offcanvasCloseFunc(selector, options) {
    ...
    setTimeout(() => {
      selector.offcanvasContainer.classList.remove(
        options.dataPostion,
        options.dataSize,
        "show"
      );
